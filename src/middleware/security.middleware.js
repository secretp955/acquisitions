...
const securityMiddleware = async (req, res, next) => {
    try {
        ...
        if(decision.isDenied() && decision.reason.isBot()) {
            logger.warn('Bot request locked', { ip: req.ip, userAgent: req.get('User-Agent'), path: req.path});
        ...
        if(decision.isDenied() && decision.reason.isRateLimit()) {
            logger.warn('Rate limit locked', { ip: req.ip, userAgent: req.get('User-Agent'), path: req.path});

            return res.status(403).json({ error: 'Forbidden', message: 'To many requests' });
        }
